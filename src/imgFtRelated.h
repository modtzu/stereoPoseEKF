...
//	int maxNumFt = 100, int octaveLvl = 3, int contrastTh = 0.1, int edgeTh = 20, int sigma = 2.6
	 ...
	 *@input rmvFlg 0 for automatic update FT0 and FT1 by rejecting bad tracked feature, 1 for doing nothing
	 */
	void trackFeature(cv::Mat img0, cv::Mat img1, vector<cv::KeyPoint> &Ft0,  vector<cv::KeyPoint> &Ft1, vector<int>& removedID, cv::Size KltWinSize = cv::Size(50,50), int rmvFlg = 0);
